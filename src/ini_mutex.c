...
ini_mutex_error_t ini_mutex_init(ini_mutex_t *mutex)
{
    ...
    if (mutex->initialized == INI_MUTEX_INITIALIZED)
        return INI_MUTEX_ALREADY_INITIALIZED;
...
ini_mutex_error_t ini_mutex_destroy(ini_mutex_t *mutex)
{
    ...
    if (mutex->locked == INI_MUTEX_LOCKED)
        return INI_MUTEX_ERROR; // mutex is locked, cannot be destroyed
...
ini_mutex_error_t ini_mutex_lock(ini_mutex_t *mutex)
{
    ...
    if (mutex->locked == INI_MUTEX_LOCKED)
        return INI_MUTEX_SUCCESS; // already locked
...
ini_mutex_error_t ini_mutex_unlock(ini_mutex_t *mutex)
{
    ...
    if (mutex->locked == INI_MUTEX_UNLOCKED)
        return INI_MUTEX_SUCCESS; // already unlocked
#if INI_OS_WINDOWS
